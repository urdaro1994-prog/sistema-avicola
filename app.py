...
# 2. ENCABEZADO: ESCUDO Y TÍTULO AL LADO
col_logo, col_titulo = st.columns([1, 5])

with col_logo:
    # Busca la imagen local con soporte para diferentes extensiones
    if os.path.exists("ESCUDO.png"):
        st.image("ESCUDO.png", width=120)
    elif os.path.exists("escudo.png"):
        st.image("escudo.png", width=120)
    elif os.path.exists("ESCUDO.jpg"):
        st.image("ESCUDO.jpg", width=120)
    else:
        st.write("🛡️") # Icono alternativo si la imagen no está cargada en GitHub
...
with tab1:
    ...
    with st.form("form_huevos", clear_on_submit=True):
        ...
        with col2:
            ...
            # Cálculo automático
            total_huevos = (cartones * 30) + unidades_sueltas
            st.info(f"👉 **Total calculated:** {total_huevos} huevos")
