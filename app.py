...
def cargar_registros_diarios():
    ...
    return df
        ...
        str_app.subheader("📝 Registro Diario - Control Zootécnico Galpón 1 (Hy-Line)")
        
        # Población inicial de referencia para el Galpón 1 (configurable según aves actuales)
        AVES_INICIALES_GALPON_1 = 16000
        ...
        if rol_actual == "Invitado":
            ...
            tab_reg_nuevo, tab_reg_historial = str_app.tabs(["➕ Registrar Día", "📋 Historial y Formato Estilo Excel"])
            ...
            with tab_reg_historial:
                ...
                if df_hist_reg.empty:
                    ...
                    # Cálculo de columnas adicionales para visualización zootécnica exacta al Excel
                    ...
                    # Estimación de % de Postura basado en aves iniciales de referencia
                    df_hist_reg['% Postura'] = ((df_hist_reg['Total Huevos'] / AVES_INICIALES_GALPON_1) * 100).round(2)
                    
                    # Consumo de alimento en gramos por ave (Alimento en Kg * 1000 / Aves)
                    df_hist_reg['g/Ave/Día'] = ((df_hist_reg['alimento_kg'] * 1000) / AVES_INICIALES_GALPON_1).round(1)
                    
                    # Reordenar columnas para que queden idénticas al formato de control
                    columnas_ordenadas = [
