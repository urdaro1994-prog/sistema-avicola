...
def inicializar_tablas_cartera():
  ...
  try:
    ...
    cur.execute(
        "ALTER TABLE abonos_cartera ADD COLUMN IF NOT EXISTS"
        " nombre_comprobante TEXT;"
    )
  except Exception:
    conn.rollback()
...
def cargar_abonos_remision(num_remision):
  ...
  conn = get_connection()
  df = pd.read_sql_query(
      "SELECT * FROM abonos_cartera WHERE num_remision = %s ORDER BY fecha_abono"
      " DESC",
      conn,
      params=(num_remision,),
  )
...
def registrar_venta_multiple(
  ...
  for item in items_venta:
    ...
    cur.execute(
        f"SELECT {clasificacion} FROM inventario WHERE galpon = %s",
        (galp_origen,),
    )
    res = cur.fetchone()
    ...
    if cantidad > stock_actual:
      ...
      raise ValueError(
          ...
          f" Stock actual: {stock_actual:,}, solicitado:"
          f" {cantidad:,}".replace(",", ".")
      )
...
def generar_pdf_registro_diario(
  ...
  resumen_data = [
      [
          Paragraph(
              f"<b>Edad Actual:</b> {current_edad_str}", style_normal
          ),
          Paragraph(
              f"<b>Saldo de Aves:</b>"
              f" {current_aves:,}".replace(",", "."),
              style_normal,
          ),
      ],
      [
          Paragraph(
              f"<b>Aves Iniciales:</b>"
              f" {aves_ini_val:,}".replace(",", "."),
              style_normal,
          ),
          Paragraph(
...
def generar_pdf_gastos(df_gastos, titulo_reporte):
  ...
  for _, fila in df_gastos.iterrows():
    ...
    col3_val = (
        str(fila.get("categoria", ""))
        if "galpon" in fila
        else str(fila.get("descripcion", ""))
    )
  ...
  totales_data = [[
      ...
      Paragraph(
          f"<b>$ {total_gasto:,.0f}</b>".replace(",", "."), style_right_bold
      ),
  ]]
  t_totales = Table(totales_data, colWidths=[374, 160])
  t_totales.setStyle(
...
if str_app.session_state.usuario_autenticado is None:
  ...
  with col_logo:
    if os.path.exists("LOGOASI.png"):
      ...
      str_app.markdown(
          "<h1 style='margin: 0;'>🥚</h1>", unsafe_allow_html=True
      )
  with col_tit:
    str_app.markdown(
    ...
    if rol_actual == "Administrador":
      ...
      if not str_app.session_state.confirmar_reinicio:
        ...
        str_app.warning(
            ...
            " abonos, clientes, gastos, registros diarios y pondrá el stock en"
            " 0."
        )
  ...
  if str_app.session_state.sesion_principal == "📦 Stock y Ventas":
      ...
      if str_app.session_state.seccion_activa == "📥 Entradas":
        ...
        if rol_actual == "Invitado":
          ...
          if str_app.button("➕ Registrar Entrada al Inventario"):
            ...
            if entradas_validas.empty:
              ...
              str_app.success(
                  "¡Entrada de inventario registrada correctamente en"
                  f" {galpon_destino}!"
              )
        ...
        if rol_actual == "Invitado":
          ...
          df_fisico_editado = str_app.data_editor(
              ...
              column_config={
                  "Stock Sistema": str_app.column_config.NumberColumn(
                      "Stock Sistema", format="%d"
                  ),
                  "Conteo Físico Real": str_app.column_config.NumberColumn(
                      "Conteo Físico Real",
                      min_value=0,
                      step=1,
                      format="%d",
        ...
        if not df_gastos.empty:
          ...
          if not df_gf.empty:
            pdf_gastos_buf = generar_pdf_gastos(
                df_gf, f"Reporte de Gastos - {mes_nombre_sel} {anio_sel_g}"
            )
        ...
        if not df_gv.empty:
          pdf_gv_buf = generar_pdf_gastos(
              df_gv, "Reporte de Gastos Varios y Personales"
          )
          str_app.download_button(
          ...
          with tab_l:
            for _, r in df_cli.iterrows():
              with str_app.expander(f"👤 {r['nombre']}"):
                ...
                if str_app.button("🗑️ Eliminar", key=f"del_c_{r['id']}"):
        ...
        if rol_actual == "Invitado":
          ...
          if (
              cliente_sel != "-- Escribir cliente nuevo --"
          ...
          df_editado = str_app.data_editor(
              ...
              column_config={
                  ...
                  "Cantidad (Huevos)": str_app.column_config.NumberColumn(
                      "Cantidad (Huevos)",
                      min_value=1,
                      step=1,
                      format="%d",
                  ...
                  "Precio Unitario ($)": str_app.column_config.NumberColumn(
                      "Precio Unitario ($)",
                      min_value=0,
                      format="$%d",
                      required=True,
                  ),
                  "Galpón Origen": str_app.column_config.SelectboxColumn(
                      "Galpón Origen",
          ...
          for _, row in df_cartera.iterrows():
            with str_app.expander(
              ...
              if not df_abonos.empty:
                df_abonos_mostrar = df_abonos[[
                    "fecha_abono",
                    "monto",
                    "nombre_comprobante",
                ]].copy()
                df_abonos_mostrar.columns = [
                    "Fecha y Hora",
                    "Monto ($)",
                    "Comprobante",
                ]
                df_abonos_mostrar["Monto ($)"] = df_abonos_mostrar[
                    "Monto ($)"
                ].apply(lambda x: f"${float(x):,.0f}".replace(",", "."))
                str_app.dataframe(
                    df_abonos_mostrar, use_container_width=True, hide_index=True
                )
              else:
          ...
          for num_sel in sorted(df_h["num_remision"].unique(), reverse=True):
            ...
            with str_app.expander(
              ...
              if not df_r.empty and columnas_validas:
                ...
                fecha_str = str(
                    f_sel.get("fecha_emision", date.today())
                )
        ...
        if df_rem.empty and df_gas.empty:
          ...
          for g in galpones:
            ...
            if (
              ...
              ingresos = float(
                  df_rem[df_rem["galpon"] == g]["total"].sum()
              )
            ...
            if (
              ...
              gastos = float(
                  df_gas[df_gas["galpon"] == g]["valor"].sum()
              )
    ...
    with tab_diario:
      if rol_actual == "Invitado":
        str_app.warning(
            "👀 Modo Invitado: No tienes permisos para registrar datos diarios."
        )
      else:
        ...
        if not config:
          ...
          with str_app.form(f"form_reg_diario_{galpon_reg}"):
            fecha_reg = str_app.date_input(
                "Fecha del Registro", value=date.today()
            )
      ...
      if not config:
        ...
        if not df_reg.empty:
          ...
          for _, row in df_reg.iterrows():
            ...
            with str_app.expander(
                f"📅 {r_fecha} | Mortalidad: {r_mort} | Conc. Consumido:"
                f" {r_cons} bultos"
            ):
