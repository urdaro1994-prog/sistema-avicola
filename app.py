...
def inicializar_tabla_inventario():
  ...
  cur.execute("""
    ...
    """)
  cur.execute(
      "INSERT INTO inventario (galpon) VALUES ('Galpón 1') ON CONFLICT (galpon)"
      " DO NOTHING;"
  )
  cur.execute(
      "INSERT INTO inventario (galpon) VALUES ('Galpón 2') ON CONFLICT (galpon)"
      " DO NOTHING;"
  )
  cur.execute(
      "INSERT INTO inventario (galpon) VALUES ('Galpón 3') ON CONFLICT (galpon)"
      " DO NOTHING;"
  )
  conn.commit()
  cur.close()
...
def inicializar_tablas_cartera():
  ...
  try:
    cur.execute(
        "ALTER TABLE abonos_cartera ADD COLUMN IF NOT EXISTS comprobante BYTEA;"
    )
    cur.execute(
        "ALTER TABLE abonos_cartera ADD COLUMN IF NOT EXISTS nombre_comprobante"
        " TEXT;"
    )
  except Exception:
    conn.rollback()
...
def cargar_gastos_varios():
  ...
  conn = get_connection()
  df = pd.read_sql_query(
      "SELECT * FROM gastos_varios ORDER BY fecha DESC, id DESC", conn
  )
  conn.close()
  if not df.empty:
    df["fecha"] = pd.to_datetime(df["fecha"]).dt.date
...
def registrar_entrada_inventario(galpon, items_entrada):
  ...
  for item in items_entrada:
    ...
    cur.execute(
        f"UPDATE inventario SET {clasificacion} = {clasificacion} + %s WHERE"
        " galpon = %s",
        (cantidad, galpon),
    )
  conn.commit()
...
def cargar_cartera():
  ...
  conn = get_connection()
  df = pd.read_sql_query(
      "SELECT * FROM cartera ORDER BY num_remision DESC", conn
  )
  conn.close()
  return df
...
def registrar_abono(
    num_remision, monto_abono, comprobante_bytes=None, nombre_comprobante=None
):
  inicializar_tablas_cartera()
  conn = get_connection()
  cur = conn.cursor()
  ...
  comp_binary = (
      psycopg2.Binary(comprobante_bytes) if comprobante_bytes else None
  )
  ...
  if res:
    total = float(res[0])
    cur.execute(
        "SELECT COALESCE(SUM(monto), 0) FROM abonos_cartera WHERE num_remision ="
        " %s",
        (num_remision,),
    )
...
def registrar_venta_multiple(
  ...
  for item in items_venta:
    ...
    datos_reales = {
        k: v for k, v in datos_insert.items() if k in columnas_validas
    }
    ...
    if datos_reales:
      ...
      cur.execute(
          f"INSERT INTO remisiones ({cols}) VALUES ({placeholders})", vals
      )

    cur.execute(
        f"UPDATE inventario SET {clasificacion} = {clasificacion} - %s WHERE"
        " galpon = %s",
        (cantidad, galp_origen),
    )
...
def generar_pdf_remision(
    num_remision, fecha_str, conductor, cliente_datos, items_df, total_factura
):
  buffer = io.BytesIO()
  ...
  img_logo = (
      Image("LOGOASI.png", width=70, height=70)
      if os.path.exists("LOGOASI.png")
      else Paragraph("<b>🥚</b>", style_bold)
  )
  ...
  header_data = [
      [
          img_logo,
          Paragraph(
              "<b>AGROAVICOLA SANTA ISABEL</b><br/><font size=8>NIT."
              " 901.786.799-7<br/>Cel. 3102397244 - 3125588606</font>",
              style_normal,
          ),
          Paragraph(
              f"<b>Remisión No.</b><br/><font size=13"
              f" color='#f26822'><b>{num_str}</b></font>",
              style_right,
          ),
      ]
  ]
  ...
  for _, fila in items_df.iterrows():
    table_data.append([
        ...
        Paragraph(
            f"{int(fila['Cantidad (Huevos)']):,}".replace(",", "."), style_right
        ),
        Paragraph(
            f"$ {fila['Precio Unitario ($)']:,.0f}".replace(",", "."),
            style_right,
        ),
        Paragraph(
            f"$ {fila['Subtotal ($)']:,.0f}".replace(",", "."), style_right
        ),
    ])
  ...
  totales_data = [
      [
          ...
          Paragraph(
              f"<b>$ {total_factura:,.0f}</b>".replace(",", "."), style_right
          ),
      ],
      [Paragraph("<b>IVA</b>", style_right), Paragraph("<b>$ 0</b>", style_right)],
      [
          Paragraph("<b>Total</b>", style_right),
          Paragraph(
              f"<b>$ {total_factura:,.0f}</b>".replace(",", "."),
              style_right_bold,
          ),
      ],
  ]
...
# --- NUEVA FUNCIÓN: GENERAR PDF DE GASTOS ---
def generar_pdf_gastos(df_gastos_fil):
  buffer = io.BytesIO()
  doc = SimpleDocTemplate(
      buffer,
      pagesize=letter,
  ...
  header_data = [
      [
          ...
          Paragraph(
              "<b>AGROAVICOLA SANTA ISABEL</b><br/><font size=8>NIT."
              " 901.786.799-7<br/>Reporte de Gastos</font>",
              style_normal,
          ),
          Paragraph(
              f"<b>Fecha Reporte:</b><br/><font size=10"
              f" color='#f26822'><b>{datetime.now().strftime('%Y-%m-%d')}</b></font>",
              style_right,
          ),
      ]
  ]
  ...
  table_data = [[
      ...
      Paragraph("Galpón", style_th),
      Paragraph("Categoría", style_th_left),
      Paragraph("Descripción", style_th_left),
      Paragraph("Valor", style_th_right),
  ]]

  total_gasto_pdf = 0.0
  for _, fila in df_gastos_fil.iterrows():
    val = float(fila.get("valor", 0))
    total_gasto_pdf += val
    table_data.append([
        Paragraph(str(fila["fecha"]), style_normal),
        Paragraph(str(fila.get("galpon", "")), style_normal),
        Paragraph(str(fila.get("categoria", "")), style_normal),
        Paragraph(str(fila.get("descripcion", "")), style_normal),
        Paragraph(f"$ {val:,.0f}".replace(",", "."), style_right),
    ])

  t_items = Table(table_data, colWidths=[70, 80, 100, 184, 100])
  t_items.setStyle(
      TableStyle([
          ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0f2942")),
          ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
  ...
  totales_data = [[
      Paragraph("<b>Total Gastos</b>", style_right),
      Paragraph(
          f"<b>$ {total_gasto_pdf:,.0f}</b>".replace(",", "."), style_right
      ),
  ]]
  t_totales = Table(totales_data, colWidths=[434, 100])
  t_totales.setStyle(
      TableStyle([
          ("ALIGN", (0, 0), (-1, -1), "RIGHT"),
          ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
...
def cargar_config_galpon(galpon):
  ...
  cur = conn.cursor()
  cur.execute(
      "SELECT edad_semanas, edad_dias, aves_iniciales FROM galpon_config WHERE"
      " galpon = %s",
      (galpon,),
  )
  res = cur.fetchone()
...
def cargar_registros_diarios(galpon=None):
  ...
  if galpon:
    df = pd.read_sql_query(
        "SELECT * FROM registros_diarios WHERE galpon = %s ORDER BY fecha DESC,"
        " id DESC",
        conn,
        params=(galpon,),
    )
    ...
    df = pd.read_sql_query(
        "SELECT * FROM registros_diarios ORDER BY fecha DESC, id DESC", conn
    )
  conn.close()
  if not df.empty:
    df["fecha"] = pd.to_datetime(df["fecha"]).dt.date
...
def generar_pdf_registro_diario(
    galpon_nombre, config, df_reg, current_edad_str, current_aves, saldo_conc
):
  buffer = io.BytesIO()
  doc = SimpleDocTemplate(
      buffer,
  ...
  header_data = [
      [
          ...
          Paragraph(
              "<b>AGROAVICOLA SANTA ISABEL</b><br/><font size=8>NIT."
              " 901.786.799-7<br/>Reporte Registro Diario - "
              + galpon_nombre
              + "</font>",
              style_normal,
          ),
  ...
  resumen_data = [
      [
          Paragraph(f"<b>Edad Actual:</b> {current_edad_str}", style_normal),
          Paragraph(
              f"<b>Aves Actuales:</b> {current_aves:,}".replace(",", "."),
              style_normal,
          ),
      ],
      [
          Paragraph(
              f"<b>Aves Iniciales:</b> {aves_ini_val:,}".replace(",", "."),
              style_normal,
          ),
          Paragraph(
              f"<b>Saldo Concentrado:</b> {saldo_conc:,.1f} bultos",
              style_normal,
          ),
      ],
  ]
  ...
  for _, fila in df_reg.iterrows():
    table_data.append([
        ...
        Paragraph(
            f"{float(fila.get('concentrado_consumido', 0)):.1f}", style_right
        ),
...
if str_app.session_state.usuario_autenticado is None:
  ...
  with col_tit:
    str_app.markdown(
        '<h2 style="margin: 0; color: #f26822 !important;">AVÍCOLA SANTA'
        ' ISABEL</h2><p style="margin: 0;">CONTROL DE ACCESO</p>',
        unsafe_allow_html=True,
    )
  ...
  tipo_usuario = str_app.selectbox(
      "Seleccione el Tipo de Usuario", ["Administrador", "Invitado"]
  )
  ...
  with col_tit:
    ...
    str_app.markdown(
        f'<h2 style="margin: 0; color: #f26822 !important;">AVÍCOLA SANTA'
        f' ISABEL</h2><p style="margin: 0;">Sesión: <b style="color:'
        f' {color_rol};">{rol_actual}</b></p>',
        unsafe_allow_html=True,
    )
  ...
  if str_app.session_state.sesion_principal is None:
    ...
    if rol_actual == "Administrador":
      ...
      if not str_app.session_state.confirmar_reinicio:
        if str_app.button(
            "🧹 Reiniciar Sistema (Dejar en Ceros)", use_container_width=True
        ):
          str_app.session_state.confirmar_reinicio = True
          str_app.rerun()
      else:
        str_app.warning(
            "⚠️ ¿Estás completamente seguro? Esto borrará todas las remisiones,"
            " abonos, clientes, gastos, registros diarios y pondrá el stock en 0."
        )
        ...
        with c_conf1:
          if str_app.button("✅ Sí, borrar todo", use_container_width=True):
            ...
            str_app.session_state.confirmar_reinicio = False
            str_app.success(
                "¡El sistema ha sido reiniciado a ceros con éxito!"
            )
            str_app.rerun()
        with c_conf2:
  ...
  if str_app.session_state.sesion_principal == "📦 Stock y Ventas":
    ...
    if (
        "seccion_activa" not in str_app.session_state
        or str_app.session_state.seccion_activa is None
    ):
      ...
      if str_app.session_state.seccion_activa == "📥 Entradas":
        ...
        if rol_actual == "Invitado":
          ...
          str_app.caption(
              "Registre los huevos recolectados y clasificados para sumarlos al"
              " inventario del galpón correspondiente."
          )
          galpon_destino = str_app.selectbox(
              "Seleccione el Galpón de Destino",
          ...
          df_entrada_editado = str_app.data_editor(
              ...
              num_rows="dynamic",
              column_config={
                  "Clasificación": str_app.column_config.SelectboxColumn(
                      "Clasificación", options=opciones_clasif, required=True
                  ),
                  "Cantidad": str_app.column_config.NumberColumn(
                      "Cantidad (Huevos)", min_value=1, step=1, required=True
                  ),
              },
          ...
          if str_app.button("➕ Registrar Entrada al Inventario"):
            entradas_validas = df_entrada_editado[
                df_entrada_editado["Cantidad"] > 0
            ].copy()
            if entradas_validas.empty:
              str_app.warning(
                  "Debe ingresar al menos un ítem con cantidad mayor a 0."
              )
            else:
              lista_items_entrada = []
              ...
              str_app.success(
                  f"¡Entrada de inventario registrada correctamente en"
                  f" {galpon_destino}!"
              )
        ...
        if rol_actual == "Invitado":
          ...
          galpon_fisico = str_app.selectbox(
              "Seleccione el Galpón a Auditar",
              ["Galpón 1", "Galpón 2", "Galpón 3"],
          )
          df_inv_actual = cargar_inventario()
          fila_galp = (
              df_inv_actual.loc[galpon_fisico]
          ...
          df_fisico_editado["Diferencia (Merma/Faltante)"] = (
              df_fisico_editado["Stock Sistema"]
              - df_fisico_editado["Conteo Físico Real"]
          )
          str_app.dataframe(
              df_fisico_editado[[
                  "Clasificación",
        ...
        if rol_actual == "Invitado":
          ...
          with str_app.form(key="form_registrar_gasto"):
            ...
            c_galpon_g = str_app.selectbox(
                "Galpón Asociado",
                ["Galpón 1", "Galpón 2", "Galpón 3", "General / Granja"],
            )
            c_categoria_g = str_app.selectbox(
                "Categoría de Gasto",
                [
                    "Alimento",
                    "Medicamentos / Sanidad",
            ...
            c_desc_g = str_app.text_input(
                "Descripción del Gasto",
                placeholder="Ej. Compra de concentrado fase 1",
            )
            c_valor_g = str_app.number_input(
        ...
        if not df_gastos.empty:
          ...
          anio_sel_g = c_f1.selectbox(
              "📅 Año (Gastos)",
              sorted(df_gastos["Año"].unique().tolist(), reverse=True),
              key="anio_gasto",
          )
          mes_nombre_sel = c_f2.selectbox(
              "📅 Mes (Gastos)", list(meses_nombres.values()), key="mes_gasto"
          )
          mes_num_sel = [
              k for k, v in meses_nombres.items() if v == mes_nombre_sel
          ][0]

          df_gf = df_gastos[
              (df_gastos["Año"] == anio_sel_g)
              & (df_gastos["Mes_Num"] == mes_num_sel)
          ]
          str_app.markdown(
              f"**Total Gastos Mes:** ${df_gf['valor'].sum():,.0f}"
          )

          # --- BOTÓN PARA DESCARGAR GASTOS EN PDF ---
          if not df_gf.empty:
            pdf_gastos_buf = generar_pdf_gastos(df_gf)
            str_app.download_button(
                label="📥 Descargar Gastos en PDF",
                data=pdf_gastos_buf,
                file_name=(
                    f"Reporte_Gastos_{anio_sel_g}_{mes_nombre_sel}.pdf"
                ),
                mime="application/pdf",
                key="btn_descargar_gastos_pdf",
            )

          for _, row_g in df_gf.iterrows():
            with str_app.expander(
                f"📅 {row_g['fecha']} — [{row_g['galpon']}]"
                f" {row_g['categoria']}: ${float(row_g['valor']):,.0f}"
            ):
              str_app.write(f"**Desc:** {row_g['descripcion']}")
              if rol_actual == "Administrador" and str_app.button(
                  f"🗑️ Eliminar Gasto #{row_g['id']}", key=f"del_g_{row_g['id']}"
              ):
        ...
        if rol_actual == "Invitado":
          ...
          with str_app.form(key="form_registrar_gv"):
            ...
            c_cat_gv = str_app.selectbox(
                "Categoría",
                ["Personal", "Hogar", "Vehículo", "Impuestos", "Varios"],
            )
            c_desc_gv = str_app.text_input("Descripción")
            c_val_gv = str_app.number_input(
                "Valor ($)", min_value=0.0, step=1000.0, format="%.0f"
            )
            if str_app.form_submit_button("💾 Guardar Gasto Vario"):
              registrar_gasto_vario(
                  c_fecha_gv, c_cat_gv, c_desc_gv, c_val_gv
              )
              str_app.toast("¡Registrado con éxito! 🎉", icon="✅")
        ...
        if not df_gv.empty:
          for _, r in df_gv.iterrows():
            with str_app.expander(
                f"📅 {r['fecha']} — [{r['categoria']}]"
                f" ${float(r['valor']):,.0f}"
            ):
              str_app.write(f"**Desc:** {r['descripcion']}")
              if rol_actual == "Administrador" and str_app.button(
                  f"🗑️ Eliminar #{r['id']}", key=f"del_gv_{r['id']}"
              ):
        ...
        if rol_actual == "Invitado":
          ...
          opciones_cli = (
              ["-- Escribir cliente nuevo --"] + df_clientes["nombre"].tolist()
              if not df_clientes.empty
              else ["-- Escribir cliente nuevo --"]
          )
          ...
          if cliente_sel != "-- Escribir cliente nuevo --" and not df_clientes.empty:
            ...
            v_nom, v_ced, v_dir, v_tel, v_em = (
                str(d_cli.get("nombre", "")),
                str(d_cli.get("cedula_nit", "")),
                str(d_cli.get("direccion", "CHOACHI")),
                str(d_cli.get("telefono", "")),
                str(d_cli.get("email", "")),
            )
          ...
          items_validos = df_editado[
              df_editado["Cantidad (Huevos)"] > 0
          ].copy()

          if not items_validos.empty:
            items_validos["Subtotal ($)"] = (
                items_validos["Cantidad (Huevos)"]
                * items_validos["Precio Unitario ($)"]
            )
            ...
            if str_app.button("🚀 Generar Remisión"):
              ...
              } for _, r in items_validos.iterrows()]
        ...
        if df_cartera.empty:
          ...
          for _, row in df_cartera.iterrows():
            with str_app.expander(
                f"Remisión N° {int(row['num_remision']):06d} —"
                f" {row['cliente']} | Saldo: ${float(row['saldo']):,.0f}"
                f" ({row['estado']})"
            ):
              if rol_actual == "Administrador":
                with str_app.form(key=f"ab_{row['num_remision']}"):
                  monto = str_app.number_input(
                      "Abono ($)",
                      min_value=0.0,
                      max_value=float(row["saldo"]),
                      step=1000.0,
                  )
                  arch = str_app.file_uploader(
                      "Comprobante",
                      type=["png", "jpg", "jpeg", "pdf"],
                      key=f"f_{row['num_remision']}",
                  )
                  if str_app.form_submit_button("Registrar Abono"):
                    registrar_abono(
                        int(row["num_remision"]),
                        monto,
                        arch.read() if arch else None,
        ...
        if not df_h.empty:
          for num_sel in sorted(df_h["num_remision"].unique(), reverse=True):
            ...
            with str_app.expander(
                f"Remisión #{int(num_sel):06d} – {cliente_nombre}"
            ):
              columnas_deseadas = [
                  "tipo_huevo",
                  "cantidad",
              ...
              columnas_validas = [
                  col for col in columnas_deseadas if col in df_r.columns
              ]
              ...
              if not df_r.empty and columnas_validas:
                ...
                str_app.warning(
                    "No se encontraron registros o columnas válidas para esta"
                    " remisión."
                )
        else:
        ...
        str_app.caption(
            "Análisis financiero de ingresos por ventas, gastos directos y"
            " utilidad neta por galpón."
        )
        ...
        if df_rem.empty and df_gas.empty:
          str_app.info(
              "No hay datos suficientes de ventas o gastos para calcular"
              " utilidades."
          )
        else:
          ...
          for g in galpones:
            ...
            if (
                not df_rem.empty
                and "galpon" in df_rem.columns
                and "total" in df_rem.columns
            ):
            ...
            if (
                not df_gas.empty
                and "galpon" in df_gas.columns
                and "valor" in df_gas.columns
            ):
          ...
          if (
            ...
            gastos_generales = float(
                df_gas[df_gas["galpon"] == "General / Granja"]["valor"].sum()
            )
          ...
          utilidad_neta_total = total_ingresos - (
              total_gastos_galpones + total_gastos_gral
          )
          ...
          col_u2.metric(
              "Total Gastos",
              f"${(total_gastos_galpones + total_gastos_gral):,.0f}".replace(
                  ",", "."
              ),
          )
          col_u3.metric(
              "Utilidad Neta",
              f"${utilidad_neta_total:,.0f}".replace(",", "."),
          )
          ...
          df_mostrar["Ingresos ($)"] = df_mostrar["Ingresos ($)"].apply(
              lambda x: f"$ {x:,.0f}".replace(",", ".")
          )
          df_mostrar["Gastos ($)"] = df_mostrar["Gastos ($)"].apply(
              lambda x: f"$ {x:,.0f}".replace(",", ".")
          )
          df_mostrar["Utilidad ($)"] = df_mostrar["Utilidad ($)"].apply(
              lambda x: f"$ {x:,.0f}".replace(",", ".")
          )
          ...
          if gastos_generales > 0:
            str_app.info(
                "💡 Gastos Generales / Granja no asignados a un galpón"
                f" específico: $ {gastos_generales:,.0f}".replace(",", ".")
            )

  elif str_app.session_state.sesion_principal == "📝 Registro Diario":
    str_app.subheader(
        "📜 Módulo de Registro Diario (Edades, Mortalidad y Concentrado)"
    )
    galpon_reg = str_app.selectbox(
        "Seleccione el Galpón",
        ["Galpón 1", "Galpón 2", "Galpón 3"],
        key="galp_reg_sel",
    )
    tab_config, tab_diario, tab_historial = str_app.tabs([
    ...
    with tab_config:
      str_app.markdown(
          "#### Configuración Inicial del Lote (Se ingresa una sola vez)"
      )
      ...
      with str_app.form(f"form_config_{galpon_reg}"):
        c_sem = str_app.number_input(
            "Edad Inicial (Semanas)",
            min_value=0,
            value=int(ini_sem),
            step=1,
        )
        c_dias = str_app.number_input(
            "Edad Inicial (Días adicionales 0-6)",
            min_value=0,
            max_value=6,
            value=int(ini_dias),
            step=1,
        )
        c_aves = str_app.number_input(
            "Número de Aves Iniciales del Lote",
            min_value=0,
            value=int(ini_aves),
            step=10,
        )
        ...
        if str_app.form_submit_button("💾 Guardar Configuración del Galpón"):
          ...
          str_app.toast("¡Registrado con éxito! 🎉", icon="✅")
          str_app.success(
              f"¡Configuración de {galpon_reg} guardada con éxito!"
          )
          str_app.rerun()
    ...
    with tab_diario:
      if rol_actual == "Invitado":
        str_app.warning(
            "👀 Modo Invitado: No tienes permisos para registrar datos"
            " diarios."
        )
      else:
        ...
        if not config:
          str_app.warning(
              "⚠️ Primero debes configurar la Edad y Aves Iniciales en la"
          ...
          with str_app.form(f"form_reg_diario_{galpon_reg}"):
            ...
            mortalidad = str_app.number_input(
                "Mortalidad del Día (Aves muertas)",
                min_value=0,
                value=0,
                step=1,
            )
            conc_ing = str_app.number_input(
                "Concentrado Ingresado (Bultos)",
                min_value=0.0,
                value=0.0,
                step=0.5,
                format="%.1f",
            )
            conc_cons = str_app.number_input(
                "Concentrado Consumido (Bultos)",
                min_value=0.0,
                value=0.0,
                step=0.5,
                format="%.1f",
            )
            huevos = str_app.number_input(
                "Huevos Recolectados (Opcional)", min_value=0, value=0, step=1
            )
            obs = str_app.text_input("Observaciones / Novedades")

            if str_app.form_submit_button("📥 Guardar Registro Diario"):
              registrar_dia_galpon(
                  fecha_reg,
                  galpon_reg,
                  int(mortalidad),
                  float(conc_ing),
              ...
              str_app.toast("¡Registrado con éxito! 🎉", icon="✅")
              str_app.success(
                  f"¡Registro diario guardado para {galpon_reg}! La edad y el"
                  " stock de aves se han actualizado automáticamente."
              )
              str_app.rerun()
    ...
    with tab_historial:
      ...
      if not config:
        ...
        total_mortalidad = (
            int(df_reg["mortalidad"].sum()) if not df_reg.empty else 0
        )
        aves_actuales = config["aves_iniciales"] - total_mortalidad

        total_conc_ing = (
            float(df_reg["concentrado_ingresado"].sum())
            if not df_reg.empty
            else 0.0
        )
        total_conc_cons = (
            float(df_reg["concentrado_consumido"].sum())
            if not df_reg.empty
            else 0.0
        )
        saldo_concentrado = total_conc_ing - total_conc_cons

        # --- CÁLCULO DEL % DE PRODUCCIÓN ACUMULADO EN LA SEMANA ---
        if not df_reg.empty:
          df_reg["dt_fecha"] = pd.to_datetime(df_reg["fecha"])
          df_reg["anio"] = df_reg["dt_fecha"].dt.isocalendar().year
          df_reg["semana"] = df_reg["dt_fecha"].dt.isocalendar().week
          df_reg = df_reg.sort_values("dt_fecha", ascending=True)

          # Huevos acumulados en la semana ISO actual/correspondiente
          df_reg["huevos_acum_semana"] = df_reg.groupby(["anio", "semana"])[
              "huevos_recolectados"
          ].cumsum()

          # Porcentaje de producción aproximado (ejemplo basado en las aves actuales del día)
          # % Prod = (huevos recolectados / aves actuales) * 100
          df_reg["%_produccion_dia"] = (
              df_reg["huevos_recolectados"] / max(aves_actuales, 1)
          ) * 100
          # O acumulado semanal promedio/porcentaje
          df_reg["%_prod_acumulada_semana"] = (
              df_reg["huevos_acum_semana"] / max(aves_actuales, 1)
          ) * 100
        ...
        if not df_reg.empty:
          pdf_buf = generar_pdf_registro_diario(
              galpon_reg,
              config,
              df_reg,
              edad_actual_str,
          ...
          str_app.download_button(
              ...
              data=pdf_buf,
              file_name=(
                  f"Registro_Diario_{galpon_reg.replace(' ', '_')}.pdf"
              ),
              mime="application/pdf",
          )
          ...
          for _, row in df_reg.iterrows():
            ...
            r_acum_sem = int(row.get("huevos_acum_semana", 0))
            r_pct_acum = float(row.get("%_prod_acumulada_semana", 0.0))

            with str_app.expander(
                f"📅 {r_fecha} | Huevos: {r_hue} | Acum. Semana: {r_acum_sem}"
                f" ({r_pct_acum:.1f}%)"
            ):
              str_app.write(
                  f"**Huevos Recolectados:** {r_hue} (Acumulado Semanal:"
                  f" {r_acum_sem} huevos — **% Prod. Acum. Semana:**"
                  f" {r_pct_acum:.2f}%)"
              )
              str_app.write(f"**Concentrado Ingresado:** {r_ing} bultos")
              str_app.write(f"**Concentrado Consumido:** {r_cons} bultos")
              str_app.write(f"**Observaciones:** {r_obs}")
              if rol_actual == "Administrador" and str_app.button(
                  f"🗑️ Eliminar Registro #{r_id}", key=f"del_reg_{r_id}"
              ):
                eliminar_registro_diario(r_id)
                str_app.warning(
                    "Registro diario eliminado y cálculos actualizados."
                )
                str_app.rerun()
        else:
          str_app.info(
              "No hay registros diarios ingresados todavía para este galpón."
          )
